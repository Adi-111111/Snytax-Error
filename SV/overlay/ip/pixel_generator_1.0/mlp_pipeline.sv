...
module mlp_pipeline #(
    ...
    integer j;
    reg signed [47:0] r_norm, g_norm, b_norm;  // normalisation temporaries
    ...
    reg signed [47:0] l3_scaled  [H3_SIZE-1:0];  // stage 2: l3_diff[31:0] * z_scale[31:16]
    reg               l3_diff_valid;
    reg               l3_scaled_valid;
    ...
    always @(posedge clk) begin
        ...
        if (!resetn) begin
            ...
            // Stage 2: multiply by integer part of z_scale only — 32x16 = 2 DSPs (~7ns)
            if (l3_diff_valid) begin
                for (j = 0; j < H3_SIZE; j = j + 1)
                    l3_scaled[j] <= l3_diff[j][31:0] * {16'b0, z_scale[31:16]};
                l3_scaled_valid <= 1;
            end

            // Stage 3: clamp, output — shift already absorbed by using z_scale[31:16] above
