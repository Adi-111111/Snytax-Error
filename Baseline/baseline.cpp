...
void draw_slider(SDL_Renderer* r, int idx, float val, bool sel, bool axis) {
    int x0 = 80, y0 = HEIGHT + 10 + idx * 6, w = 500, h = 4;
    if (axis)      SDL_SetRenderDrawColor(r, 70,  70,  70,  255);
    else if (sel)  SDL_SetRenderDrawColor(r, 230, 230, 230, 255);
    else           SDL_SetRenderDrawColor(r, 140, 140, 140, 255);
    SDL_Rect bar = {x0, y0, w, h};
    SDL_RenderFillRect(r, &bar);
    if (axis) SDL_SetRenderDrawColor(r, 90, 90, 90, 255);
    ...
    const std::vector<float>& bg)
{
    ...
    for (int py = 0; py < HEIGHT; py++) {
        for (int px = 0; px < WIDTH; px++) {
            ...
            // per-channel normalisation — matches FPGA exactly
            int r = clamp_int((int)std::round((z[0] - z_offset[0]) * z_scale[0]), 0, 255);
            int g = clamp_int((int)std::round((z[1] - z_offset[1]) * z_scale[1]), 0, 255);
            int b = clamp_int((int)std::round((z[2] - z_offset[2]) * z_scale[2]), 0, 255);
...
int main() {
    ...
    while (running) {
        ...
        if (elapsed >= 1.0) {
            ...
            frames = 0;
            fps_start = now;
            SDL_SetWindowTitle(win, ("MLP Decision Surface | FPS: " + std::to_string((int)fps) +
