...
module pixgen_tb; 
    ...
    //Generate clock input 
    reg clk = 0; 
    always #5 clk = !clk; //flip clock signal every 5 nanoseconds

    //Generate rst input 
    reg rst = 0; 
    initial #16 rst = 1; 

    //Instantiate pixel generator module 
    wire valid, sof, eol;
    pixel_generator p1 (
        .out_stream_aclk(clk),
        .s_axi_lite_aclk(clk),
        ...
        //AXI-Lite S - not being tested in this testbench 
    ...
    always @(posedge clk) begin 
        //Check for timeout signal waiting for valid 
        ...
        if(valid && ready) begin 
            //Check for start of frame (tuser in AXI_STREAM) on first word of each frame
