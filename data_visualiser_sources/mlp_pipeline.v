...
module mlp_pipeline #(
    ...
    input                           resetn,
    input                           start,
    input [(N_FEATURES*16)-1:0]     x_flat,
    ...
    localparam [6:0] L1_N_INPUTS = N_FEATURES +2;
    localparam [6:0] L2_N_INPUTS = H1_SIZE + 2;
    localparam [6:0] L3_N_INPUTS = H2_SIZE + 2;
    ...
    wire [(H1_SIZE*32)-1:0]         l1_out_flat;
    wire                            l1_done;
    ...
    mac_layer #(
        ...
        .n_inputs(L1_N_INPUTS),  // +1 for bias cycle
        .out_flat(l1_out_flat), .done(l1_done)
    );
    ...
    wire [(H2_SIZE*48)-1:0]         l2_out_flat;  // 32 actual outputs
    wire                            l2_done;
    ...
    mac_layer #(
        ...
        .n_inputs(L2_N_INPUTS),   // +1 for bias cycle (17 cycles total)
        .out_flat(l2_out_flat), .done(l2_done)
    );
    ...
    wire [(H3_SIZE*48)-1:0]         l3_out_flat;
    wire                            l3_done;
    ...
    genvar oi;
    generate
        for (oi = 0; oi < H1_SIZE; oi = oi + 1) begin : unpack_l1
            assign l1_out[oi] = l1_out_flat[oi*32 +: 32];
        end
        for (oi = 0; oi < H2_SIZE; oi = oi + 1) begin : unpack_l2
            assign l2_out[oi] = l2_out_flat[oi*48 +: 48];
        end
        for (oi = 0; oi < H3_SIZE; oi = oi + 1) begin : unpack_l3
            assign l3_out[oi] = l3_out_flat[oi*48 +: 48];
        end
    endgenerate
    ...
    mac_layer #(
        ...
        .n_inputs(L3_N_INPUTS),   // +1 for bias cycle (17 cycles total)
        .out_flat(l3_out_flat), .done(l3_done)
    );
    ...
    genvar xfi;
    generate
        for (xfi = 0; xfi < N_FEATURES; xfi = xfi + 1) begin : unpack_x
            assign x[xfi] = x_flat[xfi*16 +: 16];
        end
    endgenerate
